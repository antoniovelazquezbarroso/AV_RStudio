...
NewExportMovs <- read_excel("data/NewExport_220701_240731.xlsx") # Movimientos 01JUL2022-31JUL2024
...
# Convierte a Date la columna Fecha, incluye ordena de movimientos del banco
Changed <- Clean %>% 
          mutate(
          ...
          select(Fecha, Importe, Codigo, Concepto, Saldo)
...
Cods <- as_tibble_col(unique(NewExportMovs$Codigo), column_name = "Codigo") %>%
  left_join(unique(read_excel("data/Cods.xlsx")), by = "Codigo") %>% 
  arrange(Codigo)
